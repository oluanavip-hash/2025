...
const Home: React.FC = () => {
  const { selectedLeague, handleAddToCart, searchTerm, leagues } = useOutletContext<HomeContext>();
  ...
  const [pageTitle, setPageTitle] = useState('Coleção Torcedor 25/26');
  const [pageDescription, setPageDescription] = useState('As melhores camisetas de time com qualidade tailandesa. Encontre a camisa do seu time favorito!');

  useEffect(() => {
    if (selectedLeague === 'all') {
      setPageTitle('Coleção Torcedor 25/26');
      setPageDescription('As melhores camisetas de time com qualidade tailandesa. Encontre a camisa do seu time favorito!');
    } else {
      const league = leagues.find(l => l.id === selectedLeague);
      if (league) {
        setPageTitle(`Coleção ${league.name} 25/26`);
        setPageDescription(`As melhores camisas do ${league.name} com qualidade tailandesa. Encontre a do seu time!`);
      }
    }
  ...
  return (
    <div className="container mx-auto py-8">
      <HeroBanner selectedLeague={selectedLeague} leagues={leagues} />
      <div className="px-4">
