...
import { Search, PlusCircle, MoreHorizontal, Calendar, ShoppingCart, Loader2 } from 'lucide-react';
import { supabase } from '../../lib/supabaseClient';
import { Order } from '../../types';

type OrderStatusUI = 'Pendente' | 'Enviado' | 'Entregue' | 'Cancelado';

const statusFilters: ('Todos' | OrderStatusUI)[] = ['Todos', 'Pendente', 'Enviado', 'Entregue', 'Cancelado'];
...
const getStatusChip = (status: OrderStatusUI) => {
    switch (status) {
...
const Orders: React.FC = () => {
  ...
  const [activeFilter, setActiveFilter] = useState<'Todos' | OrderStatusUI>('Todos');
  ...
  const filteredOrders = useMemo(() => {
    return orders.filter(order => {
      ...
      const matchesFilter = activeFilter === 'Todos' || uiStatus === activeFilter;
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-4">
        <div className="flex flex-col md:flex-row items-center justify-between gap-4">
            <div className="flex border-b border-gray-200 w-full md:w-auto overflow-x-auto scrollbar-hide">
              ...
              {loading ? (
                <tr>
                  <td colSpan={7} className="text-center py-12">
                    <Loader2 size={40} className="mx-auto text-red-600 animate-spin" />
                    <p className="text-gray-500 font-medium mt-4">Carregando pedidos...</p>
                  </td>
                </tr>
                ...
                <tr>
                  <td colSpan={7} className="text-center py-12">
                    ...
                    <p className="text-sm text-gray-400 mt-1">Os novos pedidos aparecerão aqui quando forem feitos.</p>
                  </td>
                </tr>
              ) : (
                filteredOrders.map((order) => {
                  ...
                  return (
                    <tr key={order.id} className="bg-white border-b hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4">
                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${getStatusChip(uiStatus)}`}>
                          {uiStatus}
                        </span>
                      </td>
                      <td className="px-6 py-4 font-medium text-gray-800">R$ {order.total_amount.toFixed(2).replace('.', ',')}</td>
